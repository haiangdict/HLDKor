...
從 Google Sheets 抓取 HLDKor（Haiang Learner's Dictionary of Korean）的詞典資料，
...
HLD-Etymology 這份資料跨辭典共用（跟 HLDDru、HLDPwn 讀同一份試算表），
其餘三份（Lemmata/Senses/Examples）是 HLDKor 專屬的資料。
"""
...
SHEETS = [
    {
        ...
        'sheet_name': 'KOR-Lemmata',
        'output': 'data/KOR-Lemmata.csv',
    },
    {
        'id_env': 'SPREADSHEET_ID_SENSES',
        'sheet_name': 'KOR-Senses',
        'output': 'data/KOR-Senses.csv',
    },
    {
    ...
    },
    {
        ...
        'sheet_name': 'KOR-Examples',
        'output': 'data/KOR-Examples.csv',
    },
]
...
def fetch_sheet_values(gc, spreadsheet_id, sheet_name):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            return ws.get_all_values()  # 保留儲存格顯示文字，不轉型成數字
